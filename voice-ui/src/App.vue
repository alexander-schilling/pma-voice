  ...
  <body>
    ...
    <div v-if="voice.uiEnabled" class="voiceInfo">
      ...
      <p v-if="voice.voiceModes.length" :class="{ talking: voice.talking }">
        {{ voice.voiceModes[voice.voiceMode][1] }} [Rango]
      </p>
      <p
      ...
      <ul
        v-if="
          voice.radioEnabled &&
            ...
            voice.radioPlayers.length
        "
      >
        <li
          v-for="playerData in voice.radioPlayers"
          :key="playerData.id"
          :class="{
            talking: playerData.isSource
              ? voice.usingRadio
              : playerData.talking,
          }"
        >
          <span>[{{ playerData.id }}] {{ playerData.name }}</span>
        </li>
      </ul>
    </div>
  </body>
  ...
  setup() {
    ...
    window.addEventListener("message", function(event) {
      ...
      if (data.radioPlayers !== undefined) {
        voice.radioPlayers = JSON.parse(data.radioPlayers);
      }

      if (data.addRadioPlayer !== undefined) {
        let playerData = JSON.parse(data.addRadioPlayer);
        voice.radioPlayers.push(playerData);
      }

      if (data.removeRadioPlayer !== undefined) {
        let playerId = data.removeRadioPlayer;
        let index = voice.radioPlayers.findIndex(
          (player) => player.id === playerId
        );
        voice.radioPlayers.splice(index, 1);
      }

      if (data.updateRadioPlayer !== undefined) {
        let playerData = JSON.parse(data.updateRadioPlayer);
        let index = voice.radioPlayers.findIndex(
          (player) => player.id === playerData.id
        );
        voice.radioPlayers[index] = playerData;
      }
    });
